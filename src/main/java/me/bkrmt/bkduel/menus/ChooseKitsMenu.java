...
public class ChooseKitsMenu {
    ...
    public static void showGUI(Duel duel, Arena bindingArena, int pageNumber) {
        ...
        boolean isExpanded = false/*BkDuel.getInstance().getConfig().getBoolean("expanded-kit-list")*/;
        ...
        ItemBuilder removeKit = new ItemBuilder(Utils.createItem(Material.GLASS, true,
                tempName,
                Collections.singletonList(BkDuel.getInstance().getLangFile().get(duel.getFighter1(), "gui-buttons.no-kit.description")))).setUnchangedName(tempName);
...
//                                page.setUnclickable(finalIndex, true, ChatColor.GREEN + ChatColor.stripColor(page.getItems().get(event.getSlot()).getPageItem().getItem().getItemMeta().getDisplayName()), lore);
                            ...
                            if (kits.size() > 0) {
                                ...
                                if (!duel.getOptions().contains(DuelOptions.BOUND_KIT_SELECTION)) {
                                    if (kit.getPrice() == 0 || kit.isOwner(duel.getFighter1()) && !duel.getOptions().contains(DuelOptions.EDIT_MODE)) {
                                        ...
                                        else extraLore.add(BkDuel.getInstance().getLangFile().get(duel.getFighter1(), "info.kit-bought-lore"));
                                    } else {
                                        extraLore.add(" ");
                    ...
                    if (duel.getOptions().contains(DuelOptions.EDIT_MODE)) {
                        ...
                        page.pageSetItem(31, new ItemBuilder(Utils.createItem(BkDuel.getInstance().getHandler().getItemManager().getWritableBook(), true, temp2Name, Collections.singletonList(BkDuel.getInstance().getLangFile().get(duel.getFighter1(), "gui-buttons.create-arena.description")))).setUnchangedName(tempName), "choose-arena-create-kit", event -> {
            ...
            if (duel.getKit() != null || duel.getOptions().contains(DuelOptions.OWN_ITEMS)) {
                ...
                if (duel.getKit() == null && !duel.getOptions().contains(DuelOptions.OWN_ITEMS)) {
                    ...
                    if (duel.getOptions().contains(DuelOptions.OWN_ITEMS)) {
                        ...
                        kit = Utils.translateColor(BkDuel.getInstance().getConfig().getString("random-selection"));
                    } else {
                        kit = Utils.translateColor(BkDuel.getInstance().getAnimatorManager().cleanText(duel.getKit().getName()));
                    }
                ...
                double duelCost = BkDuel.getInstance().getConfig().getDouble("duel-cost");

                page.pageSetItem(NEXT_SLOT, nextButton, "choose-kits-next-button", event -> {
                    if (BkDuel.getInstance().isInvalidChallenge(duel.getFighter1(), duel.getFighter2(), duelCost)) return;
